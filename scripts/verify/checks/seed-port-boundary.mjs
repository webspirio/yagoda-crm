 ...
 * «реєстр відтворює `OPERATORS` поточково, а `base` і `all` записів не мають», а
 * `auth-mock.test.ts` — що мок-сервер розрізняє три причини відмови. Заборонити їм імпорт
 * означало б заборонити ту єдину перевірку, яка ці модулі й тримає.
 *
 * ⚠️ ДО 28.08.2026 ТУТ СТОЯВ ІНШИЙ ТЕСТ — «`signerFor()` відтворює старий мапінг ключ у
 * ключ». Тієї функції немає з `calc.ts` (фаза 4 її видалила), тобто обґрунтування винятку
 * посилалося на перевірку, якої вже не існує.
 *
...
const PORTS = [
  {
    ...
    hint: [
      ...
      '    PRODUCTS         → products',
      // ⚠️ ЦІ ДВА РЯДКИ ПЕРЕПИСАНО 28.08.2026. Раніше вони радили `signerFor(users, x)`
      // і `ownerName(users)` — обох функцій немає з `calc.ts` (фаза 4), тобто єдиний
      // текст, який людина бачить у момент падіння межі, вчив писати код, що не
      // компілюється. Гірше за це був його ЗМІСТ: «запасний підпис лишається за місцем
      // виклику» — дослівно та практика, яку `23 §Р4-8` скасував.
      '    OPERATORS[x] / OWNER → users зі знімка (імʼя ставить стор із сесії,',
      '                           запасного підпису немає — 23 §Р4-8)',
    ],
  },
  {
    name: 'auth-mock',
