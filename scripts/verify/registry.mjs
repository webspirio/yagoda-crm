...
export const CHECKS = [
  ...
  },
  {
    ...
    proves:
      ...
      'фінальної рецензії гілки: 577 тестів у 17 файлах (було 574, до фази 4 — 553 у 15), ' +
  ...
  },
  {
    ...
    proves:
      ...
      'і 28.08.2026, після виправлень за фінальною рецензією гілки, він уже 23 654 байти ' +
      'gzip і 85 592 raw.',
    blindSpot:
      ...
      'ПІСЛЯ ВИПРАВЛЕНЬ ЗА ФІНАЛЬНОЮ РЕЦЕНЗІЄЮ (попередня мірка була від 27.08: 334,8 KiB ' +
      'gzip проти теперішніх 336,9 — відтоді додався екран входу з сесією в сторі, а ' +
      'сьогодні ще й дві видимі відмови на SettleDialog і CostOfDayPage): у ' +
      'dist/assets 14 файлів (пʼять спільних і девʼять ленивих екранів), разом 336,9 KiB ' +
      'gzip (344 986 байтів) і 1 196,4 KiB raw (1 225 128), з них перший чанк 136,2 KiB ' +
      'gzip, DashboardPage 103,4, спільний bits 38,4 і css 16,5. Те, що приймальник на ' +
      'точці реально тягне на першому відкритті (index + bits + css + es2015 + ' +
      'react-dom), — 202,8 KiB gzip (207 687 байтів), тобто 60 % суми; «Перекази» (5,9) у ' +
      ...
      '(2 185 байтів gzip) лежить майже цілком у тому, що приймальник тягне на першому ' +
      'відкритті: 200,9 -> 202,8 KiB. І ЩЕ 208,8 KiB ' +
      ...
      'фаза 4 разом із виправленнями за рецензією забрала з нього 2 185 байтів gzip і ' +
      '7 732 raw, і жодного слова про це рядок у виводі справді не сказав — він лишився ' +
      'зеленим.',
  },
  {
    id: 'smoke',
  ...
  },
  {
    ...
    blindSpot:
      ...
      'фінальною рецензією (npm run coverage): src/lib — 94,86 % рядків (1 310 з 1 381) і ' +
      '86,85 % гілок (984 з 1 133); .tsx — 0,00 % (0 з 1 845 рядків у 42 файлах), тобто ' +
      'рівно нуль і після екрана входу. ОБИДВА ВІДСОТКИ ПО src/lib ВПАЛИ проти 27.08.2026 ' +
      '(95,26 і 88,45), хоч фаза 4 й принесла два повністю покриті модулі (auth.ts, ' +
      'auth-mock.ts): непокрите зосереджене в store.ts — 86,74 % рядків (301 з 347) і ' +
      '76,49 % гілок (270 з 353), найнижче в усьому src/lib. Три тести, дописані за ' +
      'рецензією, зрушили гілки src/lib на +0,22 п. п. (86,63 -> 86,85) — приріст видно, і ' +
      'він малий: гейт точки коштує кількох гілок, а не десятків. Поріг для .tsx — ' +
