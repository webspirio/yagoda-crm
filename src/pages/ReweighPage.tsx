...
function ReweighScreen() {
  ...
  function confirmVoid(id: string) {
    ...
    if (!doc) {
      /*
       * ТРИ ПРИЧИНИ, А НЕ ДВІ. Третю («документ уже сторновано») додала хвиля виправлень за
       * фінальною рецензією гілки, а текст лишився двопричинним — тобто на ній він БРЕХАВ
       * би, називаючи людині не те правило. Сьогодні недосяжно: кнопки «Сторнувати» для
       * сторнованого документа не малюють. Але саме з такої конструкції — «відмова
       * перелічує причини, яких у сторі вже не стільки» — і виросла знахідка 1 тієї ж
       * рецензії, тому текст доведено до стору, а не лишено «поки недосяжним».
       */
      toast.error('Сторно не пройшло', {
        description: 'Сторнує лише керівник, причина обовʼязкова, і лише поки документ не сторновано.',
      })
      return
    }
    setVoidingId(null)
