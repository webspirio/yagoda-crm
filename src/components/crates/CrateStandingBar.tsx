 ...
 *
 * Тому на повній сторінці «Ящики» під трьома числами стоїть четвертий рядок — сама
 * тотожність `I61` («800 = 341 + 195 + 264»). Він нічого не додає до даних і саме тому
 * потрібен: це та «сумарність», яку вона просила бачити очима, а не перевіряти в голові.
 * У компактному варіанті на прийомці цей рядок прибрано (issue #6) — див. нижче.
 *
 ...
 * тримає весь свій день на одному екрані. ВАРІАНТ, А НЕ ДРУГИЙ КОМПОНЕНТ: той самий код
 * малює наділ, смужку й три числа, тому другий рендерер не розійдеться з першим мовчки.
 * Компактний варіант знімає хром сторінки — картку-обгортку, рядок «з якої дати й хто
 * призначив» і блок «Не хватає до наділу», — а на прийомці ще й рядок-тотожність
 * `800 = 341 + 195 + 264` (issue #6, «Компактніше»): «сумарність» клієнтка просила бачити
 * очима саме на «Ящиках» (1128–1129), а на прийомці ті самі чотири числа вже стоять над
 * ним — у заголовку наділу й трьох фігурах. Смужка, три числа й червоне попередження про
 * пробитий наділ лишаються ті самі.
 */
export function CrateStandingBar({
  ...
  return (
    <div className={compact ? undefined : 'rounded-xl bg-card p-5 ring-1 ring-foreground/10'}>
      ...
      <div className={cn('grid', compact ? 'mt-2 grid-cols-3 gap-2' : 'mt-4 gap-3 sm:grid-cols-3')}>
        <Figure
