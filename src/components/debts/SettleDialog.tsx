...
export function SettleDialog({
  ...
  function submit() {
    ...
    // із сесії, а «немає кого підписати» — це відмова, названа вголос нижче.
    ...
    /*
     * ВІДМОВА ВИДИМА, А ВІКНО ЛИШАЄТЬСЯ ВІДКРИТИМ. До 28.08.2026 тут стояв безумовний
     * `onOpenChange(false)`, а тост показувався лише на успіх: на будь-якій відмові
     * `addPayout` вікно просто закривалося, грошей не виходило, а на екрані не було нічого.
     * Це «мовчазна кнопка» в найдорожчому місці застосунку — видачі готівки: касир бачить
     * закрите вікно і вважає операцію проведеною (`06 §5.3` — UI лише ПОВТОРЮЄ рішення
     * рушія, а не ковтає його). Форма відмови та сама, що в `CashFloatDialog`: заголовок,
     * опис із названими причинами, ранній вихід.
     *
     * Причин рівно чотири, і всі названі: немає сесії; це чужа точка (`actorAt`);
     * `checkBerryPayout` не пропустив суму; закривати нічого (`allocations` порожні).
     */
    if (!payout) {
      toast.error('Видачу не проведено', {
        description:
          'Гроші виходять із каси ЗА ЯГОДУ тієї точки, де ягоду прийняли, і лише в того, ' +
          'хто на цій точці стоїть. Перевірте, чи вистачає каси за ягоду і чи є що закривати.',
      })
      return
    }
    onOpenChange(false)
