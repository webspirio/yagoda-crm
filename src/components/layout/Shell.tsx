...
function PointSelect() {
   ...
   * цього файлу, де роль читається (`:92`, `:201`, `:331`, `:382` — перераховано
   * 28.08.2026), це ЄДИНЕ, де `else` віддає БІЛЬШЕ, а не менше: тут він малює селектор
   * УСІХ точок. На `role === null` вигляд ШИРШАВ БИ — рівно та підміна, від якої застерігає
   * `canActOnPoint`.
   *
   * У решті ТРЬОХ їх не чіпали, і причина не всюди однакова. На `:92` без сесії список
   * пунктів порожній, на `:201` заголовок сайдбара падає до назви точки — обидва віддають
   * менше. А `:428` (усередині `:382`) чесно виняток: його `else` друкує «приймальник · …»,
   * тобто на `role === null` СТВЕРДЖУЄ роль. Недосяжно через гейт `App.tsx` (без сесії
   * малюється екран входу, а не Shell), і саме тому лишено як є.
   *
   ...
   * комітом. Про `:428` теж було сказано «віддає менше» — неправда, див. вище.
   */
  if (role !== 'owner') {
    const point = points.find((p) => p.id === activePointId)
    return (
