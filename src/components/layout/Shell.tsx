...
function PointSelect() {
  ...
  /*
   * ⚠️ УМОВА ПЕРЕВЕРНУТА: `role !== 'owner'`, а не `role === 'operator'`. З ЧОТИРЬОХ місць
   * цього файлу, де роль читається (`NavList`, `SidebarInner`, `PointSelect` і сам
   * `Shell`), це ЄДИНЕ, де `else` віддає БІЛЬШЕ, а не менше: тут він малює селектор
   * УСІХ точок. На `role === null` вигляд ШИРШАВ БИ — рівно та підміна, від якої застерігає
   * `canActOnPoint`.
   *
   * У решті ТРЬОХ їх не чіпали, і причина не всюди однакова. У `NavList` без сесії список
   * пунктів порожній, у `SidebarInner` заголовок сайдбара падає до назви точки — обидва
   * віддають менше. А підпис «керівник · усі точки» / «приймальник · …» у самому `Shell`
   * чесно виняток: його `else` СТВЕРДЖУЄ роль на `role === null`. Недосяжно через гейт
   * `App.tsx` (без сесії малюється екран входу, а не Shell), і саме тому лишено як є.
   *
   * ⟡ ДО 28.08.2026 тут стояло «з пʼятьох місць… у решті чотирьох», і обидва числа були з
   * `main`: у цій гілці `DemoPanel` роль не читає ВЗАГАЛІ, бо його перебудували цим же
   * комітом. Про той підпис теж було сказано «віддає менше» — неправда, див. вище.
   *
   * ⟡ А НОМЕРІВ РЯДКІВ ТУТ БІЛЬШЕ НЕМАЄ — друга рецензія гілки (знахідка 6) показала, що
   * два з пʼятьох указували не туди, і рівно на +10: на стільки виріс САМ ЦЕЙ КОМЕНТАР
   * після того, як числа зміряли. Іронія в тому, що він і був виправленням попередньої
   * помилки в числах у цьому ж місці. Імена компонентів такого не вміють.
   */
  if (role !== 'owner') {
    const point = points.find((p) => p.id === activePointId)
    return (
