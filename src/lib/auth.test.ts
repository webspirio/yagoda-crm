...
describe('scopeAfterSignIn', () => {
  ...
  /*
   * ПРИЙНЯТЕ ВІДХИЛЕННЯ, ЯКЕ ДО 28.08.2026 НЕ МАЛО ЖОДНОГО ТВЕРДЖЕННЯ (знахідка 15
   * фінальної рецензії). Рішення описане в `auth.ts` абзацом на десять рядків, у `23 §12`
   * — відхиленням 1, і перевірялося рівно нічим: у цьому `describe` було два тести, обидва
   * на правильні записи.
   *
   * Суть: приймальник без точки отримує `''`, а НЕ `'all'`. Порожній рядок не збігається з
   * жодним `point.id`, тому зламаний запис реєстру не бачить нічого — а `'all'` дав би йому
   * ШИРШИЙ обсяг, ніж правильному приймальникові. Зламані дані мусять програвати. Те саме
   * правило й на межі `merge` (`store.ts`), і в `canActOnPoint` (тест вище).
   */
  it('приймальник БЕЗ точки — порожня область, а не «усі точки»', () => {
    expect(scopeAfterSignIn({ ...P1, pointId: undefined })).toEqual({
      activePointId: '',
