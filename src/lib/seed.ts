 ...
 * реєстру.
 *
 * `login` кожного приймальника дорівнює `pointId` — короткий навмисно, його набирають
 * руками на точці (`23-accounts-spec.md §3.1`). Керівницький — `'owner'`.
 */
const USERS: User[] = [
  { id: 'u_owner', name: OWNER, role: 'owner', login: 'owner' },
  ...
  /*
   * СЬОМИЙ ЗАПИС — ДРУГИЙ КАСИР ШИПИНОК, і він тут не для краси показу. Клієнт назвала
   * склад людей поточково: «на точці Гайове один касир, точка Шипинки два касири, а в
   * Попівцях один касир» (дзвінок №4, ряд. 558). Без другого запису на p1 підміну — те, що
   * критерій приймання перевіряє жестом, — нічим показати: виходити й заходити нікуди.
   *
   * Імʼя вигадане, як і решта пʼяти: публічний білд не показує жодного справжнього.
   *
   * ⚠️ СТОЇТЬ ПІСЛЯ згенерованих із `OPERATORS` навмисно: `seed.test.ts` звіряє, що реєстр
   * відтворює `OPERATORS` поточково, і порядок тут читабельний саме такий.
   */
  { id: 'u_p1b', name: 'Марія С.', role: 'operator', pointId: 'p1', login: 'p1b' },
]
