 ...
 *    типу, а зміна ЗНАЧЕНЬ у 77 літералах підпису в 13 тестових файлах плюс усі документи
 *    сіду. ЧИСЛО ЗМІРЯНЕ 28.08.2026, і саме тому воно тут одне: доти в цьому абзаці стояло
 *    «~61 у 12», а в `23 §9 Б1` — «95 у 12», і обидва подавалися як мірка (знахідка 7
 *    другої рецензії гілки). Команда, якою зміряно, — та сама в обох місцях:
 *    `git grep -n -E "'(Керівник|Оксана Г\.|Тарас Б\.|Ігор В\.|Богдан Р\.|Леся М\.|Марія С\.)'"
 *    -- '*.test.ts' '*.spec.ts' | grep -vE ':[0-9]+: *(\*|//)'` (другий `grep` викидає
 *    згадки в коментарях — вони не є значеннями, які довелося б міняти). Другий аргумент, який стояв тут до 28.08.2026, скасований цією ж фазою:
