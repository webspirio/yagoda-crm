...
describe('дві книги в одній шухляді (I57, ряд. 1104)', () => {
  ...
  /*
   * ДЗЕРКАЛО ДО РЯДКА ВИЩЕ, і воно тут не для симетрії. Мутаційна рецензія тестів зняла
   * `r.pointId === pointId` з фільтра ПОВЕРНЕНЬ — і всі 523 тести лишилися зеленими: у
   * цьому файлі жодне `back()` не мало чужої точки, а в сіді повернення є тільки на `p1`.
   * Тобто половина фільтра трималася на тому, що ніхто не приніс документа з іншої точки.
   */
  it('повернення чужої точки нашої шухляди НЕ зменшує: 23 400,00, а не 13 800,00', () => {
    const s = stand({
      issues: [issue('2026-07-30', 195)],
      returns: [back('2026-08-01', 80, { pointId: 'p3' })],
    })
    expect(s.crateCash).toBe(23_400)
    expect(s.crateCash).not.toBe(13_800)
  })
})
...
describe('вікна двох книг — різні навмисно', () => {
  ...
  // Друга половина тієї самої межі: у повернень вона трималася так само ні на чому —
  // жодне `back()` у цьому файлі не було датоване пізніше за обраний день.
  it('повернення ЗАВТРАШНЬОГО дня сьогоднішньої шухляди не чіпає: 23 400,00', () => {
    const s = stand({ issues: [issue('2026-07-30', 195)], returns: [back('2026-08-05', 80)] })
    expect(s.crateCash).toBe(23_400)
  })
})

/*
 * ОБХІД УСІХ ФІЛЬТРІВ `cashStanding()` ОДНИМ ПИТАННЯМ: чи є в наборі документ ЧУЖОЇ
 * ТОЧКИ або ЧУЖОГО ДНЯ саме для ЦЬОГО масиву? Питання поставлено до кожного з семи
 * фільтрів по черзі, а не вгадано; відповідь «ні» була у чотирьох — два повернення вище
 * і два наступні. Решта трьох (квитанції, перекази, видачі) уже мали своїх свідків.
 */
describe('прогалини рецензії тестів (каса): фільтри, яких ніхто не пробував зняти', () => {
  it('наділ ЧУЖОЇ точки не відкриває нашу книгу раніше — і не обнуляє її початок', () => {
    // Книга p1 починається 01.08, бо раніше наділу в неї не було. Наділ p3 із 27.06 у цій
    // згортці не має права нічого означати: без фільтра за точкою книга відкрилася б
    // 29.07 з НУЛЬОВИМ початковим залишком (наділу p1 на той день ще немає), і видатки
    // 31.07 пішли б у неї повністю.
    const s = stand({
      floats: [
        float({ id: 'f1', amount: 500_000, effectiveFrom: '2026-08-01' }),
        float({ id: 'f3', pointId: 'p3', amount: 50_000, effectiveFrom: '2026-06-27' }),
      ],
  ...
  it('виплата ЧУЖОЇ точки з нашої шухляди не виходить: 500 000,00 на місці', () => {
    // Квитанція чужої точки свого свідка мала («чужа точка не підмішується»), а виплата —
    // ні: жодного `payout()` з іншим pointId у цьому файлі не було.
    const s = stand({ payouts: [payout('2026-08-02', 313_353.92, '2026-07-20', { pointId: 'p3' })] })
    expect(s.berryOut).toBe(0)
    expect(s.berryCash).toBe(500_000)
...
describe('прогалини рецензії пʼятьох (каса)', () => {
  it('переказ зараховується днем ПРИЙНЯТТЯ, а не днем відправлення', () => {
    // Виїхав увечері 03.08, прийнятий уранці 04.08 — «це не півтори години» (1014).
    const t = transfer({ date: '2026-08-03', cash: 150_000, status: 'accepted', acceptedDate: '2026-08-04' })
    expect(stand({ date: '2026-08-03', transfers: [t] }).cashIn).toBe(0)
    expect(stand({ date: '2026-08-04', transfers: [t] }).cashIn).toBe(150_000)
    expect(stand({ date: '2026-08-04', transfers: [t] }).cashInToday).toBe(150_000)
  })

  it('старий документ без дати прийняття падає на день відправлення, а не зникає', () => {
    const legacy = transfer({ date: '2026-08-04', cash: 150_000, status: 'accepted' })
    expect(stand({ transfers: [legacy] }).cashIn).toBe(150_000)
  })
})
