...
export const useStore = create<State>()(
  persist(
    (set, get) => ({
       ...
       * перевіряти решту стору. Перераховано скриптом по тілах команд `store.ts` 28.08.2026:
       * жодного звертання до сесії, ролі чи точки не мають `addSupplier` (:333),
       * `updateSupplier` (:340), `updateTareType` (:345), `updateSettings` (:350),
       * `removeExpense` (:670), `setExpensePolicy` (:683) і `syncAll` (:1153). Восьмою поруч
       * стоїть `resetDemo` (:1166): сесію вона читає, але лише щоб узяти область — відмовити
