  ...
  persist(
    (set, get) => ({
      ...
      voidReweigh: (id, reason) => {
        const st = get()
        /*
         * ГЕЙТ РОЛІ. Його тут НЕ БУЛО до 28.08.2026, і це знайшла фінальна рецензія гілки:
         * `addReweigh` роль питає, а сторно того самого документа — ні. Тобто будь-хто з
         * дійсною сесією міг сторнувати переважування керівника, і в `voidedBy` лягало його
         * імʼя. Дірку тримало виключно те, що «Переважування» не показують приймальникові —
         * саме той спосіб, який `03 §UC-29 п.1` називає недостатнім («сховати пункт меню
         * недостатньо»), а `23 §6` вимагає гейта в РОЗРАХУНКУ.
         *
         * Гірше за саму дірку було те, що екран уже стверджував протилежне: `ReweighPage`
         * на відмову друкує «Сторнує лише керівник» — правило, якого в сторі не існувало.
         * Обидва сусідні сторно того самого класу його мають: `voidTransfer` і `voidCrateDoc`.
         */
        if (roleOf(st.users, st.session) !== 'owner') return undefined
        ...
        // Друге сторно того самого документа переписало б `voidedBy` і причину — тобто
        // слід указав би не ту людину. `voidCrateDoc` відмовляє на `found.voidedDate`
        // рівно з цієї причини, і на це є тест; тут відмови не було.
        if (found.status === 'voided') return undefined
        const doc: Reweigh = {
          ...found,
       ...
       *
       * ⚠️ ГЕЙТА НЕ МАЄ СВІДОМО — одна з СЕМИ таких команд, а не єдина. До 28.08.2026 тут
       * стояло «ЄДИНА КОМАНДА ФАЗИ 4 БЕЗ ГЕЙТА», і це була неправда, яка вчила читача не
       * перевіряти решту стору. Перераховано скриптом по тілах команд `store.ts` (маркери
       * `roleOf`/`actorAt`/`actorOf`/`signOf`/`stampOf`/`session`, коментарі вирізані):
       * жодного такого звертання не мають `addSupplier`, `updateSupplier`, `updateTareType`,
       * `updateSettings`, `removeExpense`, `setExpensePolicy` і `syncAll`. Восьмою поруч
       * стоїть `resetDemo`: сесію вона читає, але лише щоб узяти область — відмовити
       * не вміє. Перелік боргу — `23 §9 Б2`.
       *
       * ⟡ НОМЕРІВ РЯДКІВ ТУТ БІЛЬШЕ НЕМАЄ, і це виправлення, а не недбалість. Друга
       * рецензія гілки (знахідка 5) показала, що три з семи вказували не туди — рівно на
       * +8, тобто на довжину, якою САМ ЦЕЙ КОМЕНТАР виріс після того, як числа зміряли.
       * Комент, що цитує рядки власного файлу, ламає себе кожною своєю правкою; імена
       * команд унікальні, і `grep -n 'setExpensePolicy:'` знаходить рядок за секунду.
       *
       * Причина саме для цієї: вона віддає `void`, тому відмова була б НЕВИДИМОЮ — форма
       * показала б «збережено», а правило розподілу тихо лишилося б попереднім, що гірше за
       * відсутність перевірки. Гейт зʼявиться разом із поверненням значення.
       */
    ...
    }),
    {
       ...
       * 21 звуження нижче (по одному на кожен ключ `partialize`; у `session` і
       * `activePointId` їх по два — форма і звірка з реєстром). І сталося б це безслідно: `.catch` на `:433` віддає помилку в
       ...
       * `p` більше не зараховується його псевдонімом — і чекер перестає бачити ВСІ 21
       * звуження нижче. ПЕРЕМІРЯНО 28.08.2026 (`npm run ratchet:persist` на мутованій
       * межі): `??` дає 21 рядок «НОВИЙ НЕЗАХИЩЕНИЙ КЛЮЧ» — рівно всі ключі `partialize`,
       * а не 13, як тут стояло з часів, коли ключів було тринадцять. Тобто
      ...
      merge: (persisted = {}, current) => {
        ...
        return {
           ...
           *
           * `?? ''`, А НЕ `?? rawPoint` — виправлено 28.08.2026 за знахідкою 11 рецензії.
           * Запасним значенням тут стояв САМЕ ПІДПРАВЛЕНИЙ рядок: приймальник без точки в
           * реєстрі (той самий зламаний запис, заради якого `canActOnPoint` і
           * `scopeAfterSignIn` навмисно найсуворіші) лишав би собі те, що дописав у
           * сховище. Тобто єдина гілка, де звуження здавалося, — рівно та, яку два інші
           * місця цієї ж фази вважають ворожою. Порожній рядок — те саме, що віддає
           * `scopeAfterSignIn` (`auth.ts`): зламані дані мусять програвати, а не вигравати.
           * У сіді недосяжно (кожен приймальник має точку), тому це про майбутні дані.
           */
          activePointId:
            restoredUser && restoredUser.role === 'operator' && rawPoint !== restoredUser.pointId
              ? (restoredUser.pointId ?? '')
              : rawPoint,
