...
/*
 * ⚠️ ТУТ БУЛИ `signerFor()` І `ownerName()`, І ЇХ ВИДАЛЕНО (фаза 4, 28.08.2026).
 *
 * `signerFor(users, pointId)` відповідала на питання «хто підписує на цій точці». З
 * 28.08.2026 у клієнта на Шипинках ДВА касири (дзвінок №4, ряд. 558), тому однозначної
 * відповіді в цього питання більше немає: функція віддавала першого зі списку, і документ
 * другого касира виходив за підписом першого. Питання, яке має два правильні відповіді,
 * не можна лишати функції з одним поверненням.
 *
 * `ownerName(users)` була запасним підписом — НАЗВОЮ РОЛІ («Керівник») на випадок, коли
 * керівника в реєстрі немає. Тобто вона перетворювала «немає кого підписати» на
 * правдоподібний рядок, і саме тому була невидима очима (`23 §Р4-8`).
 *
 * Обидві заміняє `auth.ts:actorName(users, session)`: підпис — імʼя ЛЮДИНИ, яка зараз за
 * компʼютером, а `null` означає «немає кого підписати» і команда відмовляє. Видалення тут
 * НЕ храповиком продиктоване: `knip` зараховує імпорт із тесту, тому мертвими ці дві не
 * стали б і залишившись.
 */
